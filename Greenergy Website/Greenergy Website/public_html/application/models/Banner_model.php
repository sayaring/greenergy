...
class Banner_model extends CI_Model
{
     ...
     */
	public function bannerListCount($searchText = '')
    {
        $this->db->select('*');
    ...
    public function bannerList($searchText = '', $page, $segment)
    {
        $this->db->select('*');
        $this->db->from('tbl_banners');
        if(!empty($searchText)) {
  ...
  /**
    ...
    function updateData($data, $id)
    {
        $this->db->trans_start();  
        $this->db->where('id',$id);
        $query=$this->db->update('tbl_banners',$data);       
        $this->db->trans_complete();
        return $query; 
    }
     /**
    ...
    function deleteData($id)
    {
        $this->db->trans_start();  
        $this->db->where('id',$id);
        $query=$this->db->delete('tbl_banners');       
        $this->db->trans_complete();
        return $query; 
    }
    ...
    {
        $this->db->select('id,title,heading_two,our_mission,image');
        $this->db->from('tbl_banners');
