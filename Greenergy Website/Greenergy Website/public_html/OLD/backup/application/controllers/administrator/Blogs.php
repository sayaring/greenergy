...
class Blogs extends BaseController
{
    ...
    {
            $this->global['pageTitle'] = 'Blogs:: Listing';
            $searchText = $this->input->post('searchText','');
            $data['searchText'] = $searchText;            
            $this->load->library('pagination');            
            $count = $this->Blog_model->blogListCount($searchText);
			$returns = $this->paginationCompress("administrator/blogs/", $count, 20);
            $segment = ($this->uri->segment(3)) ? $this->uri->segment(3)*10 : 0;       
            $data['records'] = $this->Blog_model->blogsList($searchText, $returns["page"], $segment);        
        $this->loadViews("administrator/blog/index", $this->global, $data , NULL);
    }
    ...
    public function addEdit($id)
    {
        if($this->input->server('REQUEST_METHOD') == 'POST'){
            ...
            if($this->isAdmin() == TRUE)
            {
                ...
                $this->form_validation->set_rules('date','Post Date','trim|required');
                ...
                else
                {
                        ...
                        $field_name ='file';
                         ...
                         $config['upload_path']   = $rootPath; 
                          ...
                          if (!$this->upload->do_upload('file')) {
                            ...
                            $this->resizeImage($uploadedImage['file_name'], $uploadedImage['full_path'], $newFolder, 370,270);                            
                          }
                      }
                    ...
                    if($id){
                         $getData=$this->Blog_model->getBlogsList($id);
                         if(!empty($getData)){
                            ...
                            $result = $this->Blog_model->updateData($saveData, $id);   
                         } else {
                        ...
                        $result = $this->Blog_model->saveData($saveData);
                    }
            ...
            $this->global['pageTitle'] = 'Blogs :: Add & Edit';
            $this->global['pageHeading'] = 'Blogs';
            if(!empty($id)){
                $this->global['pageSection'] = 'Update :: Blogs Details';
                $data['getData'] = $this->Blog_model->getBlogsList($id);

            }else{
                $this->global['pageSection'] = 'Save :: Blogs Details';
            }
            
            $data['roles'] = $this->User_model->getUserRoles();
            $data['id'] = !empty($id) ? $id : 0;
            $this->loadViews("administrator/blog/addEdit", $this->global, $data , NULL);
        }
            ...
            $getData=$this->Blog_model->getBlogsList($id);
             if(!empty($getData)){
                if(!empty($saveData['image'])) {
                 ...
                 $result = $this->Blog_model->deleteData($id);   
            ...
            $getData=$this->Blog_model->getBlogsList($id);
             if(!empty($getData)){
                $data = array(
                 ...
                 $result = $this->Blog_model->updateData($data, $id);   
