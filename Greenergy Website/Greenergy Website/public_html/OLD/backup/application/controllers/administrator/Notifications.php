...
class Notifications extends BaseController
{
    ...
    {
            $this->global['pageTitle'] = 'Notifications:: Listing';
            $searchText = $this->input->post('searchText','');
            $data['searchText'] = $searchText;            
            $this->load->library('pagination');            
            $count = $this->Notifications_model->blogListCount($searchText);
			     $returns = $this->paginationCompress("administrator/notifications/", $count, 20);
            $segment = ($this->uri->segment(3)) ? $this->uri->segment(3)*10 : 0;       
            $data['records'] = $this->Notifications_model->blogsList($searchText, $returns["page"], $segment);        
        $this->loadViews("administrator/notifications/index", $this->global, $data , NULL);
    }
    ...
    public function addEdit($id)
    {
        if($this->input->server('REQUEST_METHOD') == 'POST'){
            ...
            if($this->isAdmin() == TRUE)
            {
               ...
               // $this->form_validation->set_rules('meta_title','Meta Title','trim|required');
                ...
                else
                {
                    ...
                    if($id){
                         $getData=$this->Notifications_model->getBlogsList($id);
                         if(!empty($getData)){
                            ...
                            $result = $this->Notifications_model->updateData($saveData, $id);   
                         } else {
                        ...
                        $result = $this->Notifications_model->saveData($saveData);
                    }
            ...
            $this->global['pageTitle'] = 'Notifications :: Add & Edit';
            $this->global['pageHeading'] = 'Notifications';
            if(!empty($id)){
                $this->global['pageSection'] = 'Update :: Notifications Details';
                $data['getData'] = $this->Notifications_model->getBlogsList($id);

            }else{
                $this->global['pageSection'] = 'Save :: Notifications Details';
            }
            
            $data['roles'] = $this->User_model->getUserRoles();
            $data['id'] = !empty($id) ? $id : 0;
            $this->loadViews("administrator/notifications/addEdit", $this->global, $data , NULL);
        }
            ...
            $getData=$this->Notifications_model->getBlogsList($id);
             if(!empty($getData)){
                if(!empty($getData['image'])) {
                 ...
                 $result = $this->Notifications_model->deleteData($id);   
            ...
            $getData=$this->Notifications_model->getBlogsList($id);
             if(!empty($getData)){
                $data = array(
                 ...
                 $result = $this->Notifications_model->updateData($data, $id);   
